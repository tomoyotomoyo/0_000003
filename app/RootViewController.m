...
- (void)runExploitStaged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [strongSelf boot:@"> Exploiting kernel (opa334 ICMPv6 OOB)..." type:@"dim"];
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                strongSelf2->_exploitRunning = NO;

                if (ret == 0) {
                    strongSelf2->_exploitDone = YES;
                    [strongSelf2 boot:@"> Kernel R/W established" type:@"success"];
                    [strongSelf2 boot:[NSString stringWithFormat:@"> Self proc: 0x%llX", KFProcSelf()] type:@"dim"];

                    [strongSelf2 boot:@"> Sandbox escaped" type:@"success"];
                    [strongSelf2 boot:[NSString stringWithFormat:@"> Root elevation OK (uid=%d)", getuid()] type:@"success"];

                    if (KFLogInitFilter(&strongSelf2->_filter)) {
                        [strongSelf2 boot:@"> Kernel log channel connected" type:@"success"];
                        [strongSelf2 setStatus:@"ok" text:@"LOGGING"];
                        [strongSelf2 startLogStream];
                    } else {
                        [strongSelf2 boot:@"> Kernel log channel failed (fallback mode)" type:@"dim"];
                        [strongSelf2 setStatus:@"warn" text:@"FALLBACK"];
                    }
                } else {
                    [strongSelf2 boot:[NSString stringWithFormat:@"> EXPLOIT FAILED: %d", ret] type:@"error"];
                    [strongSelf2 setStatus:@"err" text:@"FAILED"];
                }
            });
        });
    });
